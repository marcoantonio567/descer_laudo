...
def gerenciar_maquinario(instituicao):
    ...
    maquinarios =  selecionar_matriculas_maquinas('Maquina')    
    
    # Mapeamento das instituições para os caminhos
    caminhos = {
        'Sicredi': caminho_maquinario,
    ...
    if quantidade_maquinarios==1:
        ...
        apertar_Tab()
        escrever_texto("Maquinario")
        apertar_Tab()
        escrever_texto(nome_proponente_geral)
        apertar_Tab()
        escrever_texto(cidade)
        apertar_Tab()
        escrever_texto(maquinarios[0])#aqui digitando a area de matricula
    ...
    if quantidade_maquinarios>1:
        ...
        escrever_texto(str(instituicao))
        voltar_celula()
        mesclar_e_centralizar_celulas(quantidade_maquinarios,cordenadas_do_usuario)
        ...
        for maquina in maquinarios:
            escrever_texto(maquina)#aqui digitando a area de matricula
            time.sleep(0.2)
            enter()
            apertar_Tab()
